...
- 在同一会话内完成主密码轮换
- 在同一会话内显式 `lock` / `unlock`，验证锁屏与重新解锁边界
...
本轮提交继续推进“阶段二：终端接入层标准化”。在上一轮统一命令契约的基础上，这次进一步把 shell/未来 TUI 需要的输入阶段与失败恢复语义收敛成更细的状态机，并补上对应交互回归测试。

- 在 `src/app/frontend_contract.*` 中把原先较粗的“编辑中 / 确认中”拆成更具体的条目表单态、主密码表单态、覆盖确认态、删除确认态、主密码轮换确认态，并补充空命令输入判定与确认后状态推进规则
- `src/shell/interactive_shell.cpp` 改为复用共享的空输入判定和确认后状态切换逻辑；命令执行失败时会显式经过“恢复中”状态，再返回可继续操作的 `ready`
- 扩展 `zkvault_frontend_contract` 测试，固化空输入规则和更细粒度的状态映射；扩展 `zkvault_shell_smoke`，覆盖空输入忽略、确认阶段 EOF 取消以及重复确认失败后的状态恢复
- 这轮工作的目标是先把 shell 已经暴露出来的交互阶段明确定义清楚，再为后续全屏 TUI 复用这些状态语义和恢复规则

这次改动后，终端前端已经不再只暴露笼统的“编辑/确认/失败”概念，而是开始具备可直接映射到表单、确认框和失败恢复提示的状态边界。

## 未来需要增加的内容

当前项目已经具备稳定的保险库核心、CLI 接口和会话式 shell 原型，但距离完整 TUI 仍有明确增量需要补齐。后续建议按以下方向继续推进：

- 把这些已拆分的表单态、确认态和恢复态继续组织成显式的终端状态机与转移表，减少后续 TUI 编排时的分支散落
